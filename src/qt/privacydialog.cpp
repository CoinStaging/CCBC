                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zDOD ought to be enough for anybody." - Bill Gates, 2017
    ui->zDODpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzDODSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzDOD->setEnabled(false);
        ui->pushButtonMintzDOD->setToolTip(tr("zDOD is currently disabled due to maintenance."));

        ui->pushButtonSpendzDOD->setEnabled(false);
        ui->pushButtonSpendzDOD->setToolTip(tr("zDOD is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zDODpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzDOD_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDOD is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zDOD...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zDOD in ") +
...
void PrivacyDialog::on_pushButtonSpendzDOD_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDOD is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zDOD
        sendzDOD();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zDOD
    sendzDOD();
}

void PrivacyDialog::on_pushButtonZDodControl_clicked()
{
    ZDodControlDialog* zDodControl = new ZDodControlDialog(this);
    zDodControl->setModel(walletModel);
    zDodControl->exec();
}

void PrivacyDialog::setZDodControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDodSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDOD()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Dod Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zDODpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zDOD is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DOD </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zDOD</b>";
    ...
    // use mints from zDod selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZDodControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZDodControlDialog::GetSelectedMints();
    }
    ...
    // Clear zdod selector in case it was used
    ZDodControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDod Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Dod, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zDod Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDOD </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zDOD "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zDOD "));
    ui->labelzDODAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzDODSyncStatus->setVisible(fShow);
}
