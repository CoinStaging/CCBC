...
#include "ui_zdodcontroldialog.h"
...
std::list<std::string> ZDodControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDodControlDialog::listMints;

ZDodControlDialog::ZDodControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDodControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDodControlDialog::~ZDodControlDialog()
{
    delete ui;
}

void ZDodControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDodControlDialog::updateList()
{
...
void ZDodControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDodControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDod_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDodControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDodControlDialog::GetSelectedMints()
{
...
void ZDodControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
